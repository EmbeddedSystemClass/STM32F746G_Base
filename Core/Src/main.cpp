...
#define DBG_FATFS BSP_CLI_Printf
#define DBG_QUEUE BSP_CLI_Printf
		...
		case 0:
		{
			...
			if(ret == MSD_OK)
			{
				...
				else
				{
					...
					/* Open log file */
					fr = f_open(&fil, scFile, FA_CREATE_ALWAYS | FA_WRITE);
					if(fr == FR_OK)
					{
						fr = f_write(&fil, (void*)scText, strlen(scText), (UINT *)&ulBytesWritten);
						if(fr == FR_OK)
						{
...
{
  BSP_CLI_Printf("DHCP State change (%d >> %d)\r\n", DHCP_state, state);
  DHCP_state = state;
}
